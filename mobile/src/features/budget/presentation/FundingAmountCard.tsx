...
export function FundingAmountCard({ currency, fundingAmount, onChangeFundingAmount }: Props) {
  const [editing, setEditing] = useState(false);
  const [input, setInput] = useState(fundingAmount === null ? '' : String(fundingAmount));

  useEffect(() => setInput(fundingAmount === null ? '' : String(fundingAmount)), [fundingAmount]);
      ...
      <TouchableOpacity onPress={() => setEditing(true)} activeOpacity={0.84} style={styles.card}>
        <View style={styles.copy}>
          ...
          <Text style={styles.amount}>
            {fundingAmount === null ? 'Not set' : formatMoney(fundingAmount, currency)}
          </Text>
          <Text style={styles.support}>The amount available to shape this plan.</Text>
        </View>
        <View style={styles.editPill}>
          <MaterialIcons name="edit" size={16} color={Colors.palette.green} />
          <Text style={styles.editText}>{fundingAmount === null ? 'Set' : 'Edit'}</Text>
        </View>
      </TouchableOpacity>
      ...
      <Modal visible={editing} transparent animationType="fade" onRequestClose={() => setEditing(false)}>
        <View style={styles.backdrop}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalCopy}>How much money is available for this plan?</Text>
            <TextInput
              value={input}
...
const styles = StyleSheet.create({
  ...
  amount: { marginTop: 2, fontFamily: Fonts.headingSemiBold, fontSize: 22, lineHeight: 28, color: Colors.text.primary },
